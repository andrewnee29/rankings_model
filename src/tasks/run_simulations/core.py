...
class Pool():
    ...
    def __init__(self, teams, team_dict):
        ...
        self.stand = pd.DataFrame(
            {"Team": teams, "RD": [0] * len(teams), "GW": [0] * len(teams), "GL": [0] * len(teams),
             "Rating": [team_dict[team] for team in teams]})
...
class Simulator():
    ...
    def _find_highest(self, rowA, rowB, rowC, team_dict_w):
        ...
        if (rowA["ML"] > rowB["ML"]) & (rowA["ML"] > rowC["ML"]):
            ...
            return "C"
        elif (team_dict_w[rowA["Team"]] > team_dict_w[rowB["Team"]]) & (
                team_dict_w[rowA["Team"]] > team_dict_w[rowC["Team"]]):
            return "A"
        elif (team_dict_w[rowB["Team"]] > team_dict_w[rowA["Team"]]) & (
                team_dict_w[rowB["Team"]] > team_dict_w[rowC["Team"]]):
            return "B"
        elif (team_dict_w[rowC["Team"]] > team_dict_w[rowA["Team"]]) & (
                team_dict_w[rowC["Team"]] > team_dict_w[rowB["Team"]]):
            return "C"
        else:
            return "A"
...
class Multi_Simulator(Simulator):
    ...
    def _get_sum_pool(self, pool, write=False, tag="", n=0):
        ...
        if write == True:
            pool[["Team", "GW", "GL", "Rating"]].groupby("Team").mean().sort_values("GW", ascending=False).round(
                2).to_csv("{}_pool_{}.csv".format(tag, n + 1))
    ...
    def give_seed_summary(self, write=False, tag=""):
        ...
        # Sorts by games won
        if write == True:
            self.div_final_con[["Team", "GW", "GL", "Rating", "Seed"]].groupby("Team").mean().sort_values("GW",
                                                                                                          ascending=False).round(
                2).to_csv("{}_seeds.csv".format(tag))
        return self.div_final_con[["Team", "GW", "GL", "Rating", "Seed"]].groupby("Team").mean().sort_values("GW",
                                                                                                             ascending=False).round(
            2)
    ...
    def give_bracket_summary(self, write=False, tag=""):
        ...
        bracket = pd.DataFrame({"Round_32": [big_list[0][0]], "Round_16": [big_list[0][1]], "Round_8": [big_list[0][2]],
                                "Semis": [big_list[0][3]], "Finals": [big_list[0][4]], "Champion": [big_list[0][5]]})
        for i in range(1, len(big_list)):
            bracket = pd.concat([bracket, pd.DataFrame(
                {"Round_32": [big_list[i][0]], "Round_16": [big_list[i][1]], "Round_8": [big_list[i][2]],
                 "Semis": [big_list[i][3]], "Finals": [big_list[i][4]], "Champion": [big_list[i][5]]})]).reset_index(
                drop=True)
        ...
        for team in list(team_dict.keys()):
            ...
            indi = pd.DataFrame(
                {"Team": [team], "Round_16": [rds[0] * 100], "Round_8": [rds[1] * 100], "Semis": [rds[2] * 100],
                 "Finals": [rds[3] * 100], "Champion": [rds[4] * 100]})
            results = pd.concat([results, indi])
        if write == True:
            results.sort_values(["Champion", "Finals", "Semis", "Round_8", "Round_16"], ascending=False).round(
                2).reset_index(drop=True).to_csv("{}_bracket.csv".format(tag))
        return results.sort_values(["Champion", "Finals", "Semis", "Round_8", "Round_16"], ascending=False).round(
